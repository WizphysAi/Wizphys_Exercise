...
public sealed class BlazePosePronated : MonoBehaviour
{
    [SerializeField]
    ...
    private float CheckPBRight()
    {
        ...
        CheckPBRightCount = (DeltaRightShoulder > 12) ? (CheckPBRightCount + 1) : (CheckPBRightCount = 0);

        if (CheckPBRightCount > 12)
        {
            pb_Left_Right.text = "Pronated Bracing Right";
    ...
    private float CheckPBLeft()
    {
        ...
        CheckPBLeftCount = (DeltaLeftShoulder > 12) ? (CheckPBLeftCount + 1) : (CheckPBLeftCount = 0);

        if (CheckPBLeftCount > 12)
        {
            //Debug.Log("Pronated Bracing Left");
            PBLeftFlag = true;
            pb_Left_Right.text = "Pronated Bracing Left";
            pb_Left_Right.gameObject.SetActive(true);
        }
        else
        {
